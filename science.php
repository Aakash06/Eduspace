        ...
        $(document).ready(function () {
            ...
            $(".su").click(function () {
                ...
                if ((oa == true && ob == false && oc == false && od == false) ||
                    ...
                    $("#q" + num).css("backgroundColor", "grey");
                }
                else if (oa == false && ob == false && oc == false && od == false) {
                    $("#q" + num).css("backgroundColor", "#9C9");
                }
                else {
                    $("#q" + num).css("backgroundColor", "#9C9");
...
<div style="background-color: white; overflow:hidden; ">
    ...
    <div id="quiz screen"
         style="margin-top:40px;margin-left:auto;margin-right:auto;margin-bottom:60px;width:80%;background-color:palegoldenrod;overflow:hidden;border-radius:8px;">

        <center>
            <div id="start_quiz"
                 style="width:230px;background-color:#0C6;color:ghostwhite;border:1px solid white;border-radius:5px;padding:15px 25px;margin-top:50px;margin-bottom:50px;font-size:2.5em;cursor:pointer;">
                Start Quiz !
            </div>
        </center>

        <div class="hidekar" id="tymleft"
             style="font-size:1.5em;width:215px;margin:5px auto 25px auto;padding:5px 20px;overflow:hidden;background-color:#099;color:ghostwhite;">
            Time Left - 15:00
        </div>
        <form method="post">
            <?php
            $query = "select * from questionbank where subjectno=1 order by rand() limit 15;";
            $data = mysql_query($query);
            $flag = 1;
            while ($rows = mysql_fetch_array($data)) {

                ?>
                <div class="hidekar" id="ques<?php echo $flag ?>" data-ans="<?php echo $rows['CORRECTANS'] ?>"
                     style="margin:10px auto 5px auto;overflow:hidden;font-size:1.2em;padding:5px 20px;">
                    <b><?php echo $flag . ". " . $rows['QUES'] ?></b>
                </div>
                <div class="hidekar" id="optiona<?php echo $flag ?>"
                ...
                <div class="hidekar" id="optionb<?php echo $flag ?>"
                     style="margin-left:20px;margin-top:10px;overflow:hidden;">
                    <input type="checkbox" id="ob<?php echo $flag ?>" name="b" value="<?php echo $rows['A2'] ?>"/>&nbsp;&nbsp;<?php echo $rows['A2'] ?>
                </div>
                <div class="hidekar" id="optionc<?php echo $flag ?>"
                     style="margin-left:20px;margin-top:10px;overflow:hidden;">
                    <input type="checkbox" id="oc<?php echo $flag ?>" name="c" value="<?php echo $rows['A3'] ?>"/>&nbsp;&nbsp;<?php echo $rows['A3'] ?>
                </div>
                <div class="hidekar" id="optiond<?php echo $flag ?>"
                     style="margin-left:20px;margin-top:10px;overflow:hidden;">
                    <input type="checkbox" id="od<?php echo $flag ?>" name="d" value="<?php echo $rows['A4'] ?>"/>&nbsp;&nbsp;<?php echo $rows['A4'] ?>
                </div>
                <div id="buttons<?php echo $flag ?>" class="hidekar"
                     style="margin-top:20px;margin-left:20px;margin-right:20px;overflow:hidden">
                    <input type="button" class="su" id="submit<?php echo $flag ?>" value="Next"
                           data-num="<?php echo $flag ?>" name="su" style="float:left;"/>
                    <input type="button" class="fi" id="finish<?php echo $flag ?>" value="Finish" name="fi"
                           style="float:right;"/>
                </div>
                <?php
                $flag++;
        ...
        <div class="hidekar" id="list" style="margin:20px 10px 10px 20px;overflow:hidden;">
            <center><?php for ($i = 1; $i <= 15; $i++) { ?>
                    <span class="qlist" id="q<?php echo $i ?>"
                          style="margin:10px;background-color:#9C9;color:white;padding:10px 10px;cursor:pointer;text-align:center; border-radius:80%; font-size:1.2em;">
